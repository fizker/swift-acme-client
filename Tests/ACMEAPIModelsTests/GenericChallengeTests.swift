...
struct GenericChallengeTests {
	...
	func decode__jsonContainsVariousFields__fieldsAreDeocdedCorrectly() async throws {
		...
		let challenge = try decode(json) as GenericChallenge
	...
	@Test(arguments: [
		(
			GenericChallenge(status: .pending, allFields: [
	...
	func description__letsEncryptDNSChallenge__outputIsFormatted(challenge: GenericChallenge, expected: String) async throws {
		let actual = """
			CH:
